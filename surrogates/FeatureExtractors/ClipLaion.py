...
class ClipLaionFeatureExtractor(BaseFeatureExtractor):
    ...
    def global_local_features_visual(self, x):
        ...
        visualize_feature(x, attentions[-1], save_path="visual/laion/laion_heatmap_last.png", alpha=0.5)
        for i in range(0, len(attentions)):
            visualize_feature(x, attentions[i], save_path=f"visual/laion/laion_heatmap_{i}.png", alpha=0.5)
...
def visualize_feature(
    ...
    attn_map = attention.mean(dim=1)[0, 0, 1:].reshape(-1, 1)      # [256, 1]
    # attn_map = attention.mean(dim=1)[0, 1:, 1:].mean(dim=0).reshape(-1, 1)      # [256, 1]

    patch_w, patch_h = 16, 16
    image_w, image_h = 224, 224
    num_patches = patch_w * patch_h
    attn_map = attn_map[:num_patches].reshape(1, 1, patch_w, patch_h)   # [1, 1, 16, 16]
